...
from cogs.poll import PollMessageTrack
from configuration import TOKEN, UMBRA_ID, MAIN_CHANNEL_ID, MY_GUILD, MAX_HIST_RETRIEVE_RECORDS, PREFIX
...
class Bot(commands.Bot):
    ...
    def __init__(self):
        ...
        self.poll_track = PollMessageTrack()  # poll_track used for message stats
        self.caps = 0  # caps and nahooj variables changed in cogs.manage_users.ManageUser class
        self.nahooj = 0

    async def setup_hook(self) -> None:
        await self.load_extension("cogs.google_search")
        await self.load_extension("cogs.poll")
        await self.load_extension("cogs.translate")
        await self.load_extension("cogs.voice_message")
        await self.load_extension("cogs.message_stats")
        await self.load_extension("cogs.manage_users")
    ...
    async def on_command_error(self, ctx: commands.Context, exception: CommandError) -> None:
        ...
        if isinstance(exception, discord.ext.commands.errors.CommandOnCooldown) or \
            ...
            raise exception  # re-raise the error so all the errors will still show up in console
...
@bot.hybrid_command(name="cmds", description="Список доступных комманд")
@app_commands.guilds(MY_GUILD)
